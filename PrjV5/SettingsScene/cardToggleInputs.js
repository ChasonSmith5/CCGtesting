...
export function toggleCards(buttonNumber) {
...
function restoreInputValues(buttonNumber, numTimes) {
    for (var i = 0; i < numTimes; i++) {
        var abillityInput = getButton(buttonNumber, i, 'ABL');
        var energyTypeInput = document.getElementById('TYP' + buttonNumber + '.' + (i + 1));
        var energyInput = document.getElementById('NRG' + buttonNumber + '.' + (i + 1));
        var powerInput = document.getElementById('PWR' + buttonNumber + '.' + (i + 1));
        if (abillityInput)
            abillityInput.value = localStorage.getItem('ABL' + buttonNumber + '.' + (i + 1)) || '';
        if (energyTypeInput)
            energyTypeInput.value = localStorage.getItem('TYP' + buttonNumber + '.' + (i + 1)) || '';
        if (energyInput)
            energyInput.value = localStorage.getItem('NRG' + buttonNumber + '.' + (i + 1)) || '';
        if (powerInput)
            powerInput.value = localStorage.getItem('PWR' + buttonNumber + '.' + (i + 1)) || '';
    }
}
function getButton(buttonNumber, i, identifier) {
