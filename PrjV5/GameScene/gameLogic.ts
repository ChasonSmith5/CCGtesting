...
export function start_game(){
    ...
    for(let i = 1; i <= settingsHandler.p1cards; i++){
        var p1card = new (makeCard as any)(settingsHandler.p1pwr[i-1], settingsHandler.p1nrg[i-1], settingsHandler.p1abl[i-1], settingsHandler.p1typ[i-1]);
        gameVariables.p1deck.push(p1card);
    }
    for(let i = 1; i <= settingsHandler.p2cards; i++){
        var p2card = new (makeCard as any)(settingsHandler.p2pwr[i-1], settingsHandler.p2nrg[i-1], settingsHandler.p2abl[i-1], settingsHandler.p2typ[i-1]);
...
export function toRules(){
    window.location.href = '../RulesScene/GameBuild.htm';
}
